...
export const useStore = create<StoreState>((set, get) => ({
  ...
  onMouseDown: (mousePosition: Point) => {
    set(state => ({ ...core.onMouseDown(state, mousePosition) })),
      undefined,
      'onMouseDown'
  },
  ...
  selectShape: (shapeId: string) => {
    set(state => ({ ...core.selectShape(state, shapeId) })),
      undefined,
      'selectShape'
  },

  toggleSelected: (shapeId: string) => {
    set(state => ({ ...core.toggleSelected(state, shapeId) })),
      undefined,
      'toggleSelected'
  },

  updateShape: (shape: core.Shape) => {
    set(state => ({ ...core.updateShape(state, shape) })),
      undefined,
      'updateShape'
  },

  setSnapToGridSetting: (snapToGrid: boolean) => {
    set(state => ({ ...core.setSnapToGridSetting(state, snapToGrid) })),
      undefined,
      'setSnapToGridSetting'
  },

  deleteSelectedShapes: () => {
    set(state => ({ ...core.deleteSelectedShapes(state) })),
      undefined,
      'deleteSelectedShapes'
  }
}))
